...
def student_mark():
    if request.method == 'POST':
        ...
        new_marks = {'Maths': Maths, 'Physics': Physics, 'Chemistry': Chemistry, 'biology': Biology,'Computer':Computer_Science}
        new_data = pd.DataFrame([new_marks])
        prediction = loaded_model.predict(new_data)
        print(prediction)
        course_name=list(course_labels.keys())[prediction[0]]
        return render_template('profile.html',Maths=Maths,Physics=Physics,Chemistry=Chemistry,Biology=Biology,Computer_Science=Computer_Science,course_name=course_name)
